...
//根据需要选择模型
    ...
    pose.header.frame_id = "smallodom";
...
void Trajectoryer::target_callback(const auto_aim_interfaces::msg::Target msg)
{
    ...
    if(is_tracking)
    {
        ...
        //{
            ...
            if((needchangeyaw) < 5 && (needchangeyaw) > -5)
            {
    ...
    {
        auto_aim_interfaces::msg::SendSerial result;
        result.header.frame_id = "smallodom";
        result.header.stamp = this->now();
        result.is_tracking = is_tracking;
        result.is_can_hit = false;
        result.bigyaw = 0.0;
...
{
    if(tf2_buffer_->canTransform("smallodom", "prediction_camera_optical_frame", tf2::TimePointZero)){
      //std::cout << "Frames can be transformed" << std::endl;
    }else{
    ...
    try {
        armorps.point = tf2_buffer_->transform(armorps, "prediction_camera_optical_frame").point;
    } catch (const tf2::ExtrapolationException & ex) {
...
{
    if(tf2_buffer_->canTransform("smallodom", "bigodom", tf2::TimePointZero)){
      //std::cout << "Frames can be transformed" << std::endl;
    }else{
    ...
    enemyps.header.stamp = bigps.header.stamp;
    enemyps.header.frame_id = "base_link";
    enemyps.point = bigps.point;
